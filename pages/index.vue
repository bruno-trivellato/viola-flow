<template>
  <div class="h-screen p-3 transition-colors duration-200" :class="isDark ? 'bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'">
        ...
        >
          ...
          >
            <table class="w-full text-sm">
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-2 py-2 text-center">
                    ...
                    <button
                      ...
                      @click="overwriteExistingRow(index)"
                ...
                <!-- Empty state / Add row -->
                <tr v-if="importRows.length === 0" class="border-t" :class="isDark ? 'border-gray-700' : 'border-gray-200'">
          ...
          <div class="flex items-center justify-between pt-2 border-t" :class="isDark ? 'border-gray-700' : 'border-gray-200'">
            <div class="flex items-center gap-2">
              ...
              <button
                v-if="!isImporting && importRows.length > 0"
    ...
    <div class="flex items-center gap-3 p-3 border-b flex-wrap transition-colors duration-200" :class="isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-300 shadow-sm'">
      ...
      <!-- Settings Menu -->
      <div class="relative ml-auto" ref="settingsMenuContainer">
        <button
...
const settingsMenuContainer = ref<HTMLDivElement | null>(null)

// Import modal state
const showImportModal = ref(false)
const isImporting = ref(false)
const importTableContainer = ref<HTMLDivElement | null>(null)
...
interface ImportRow {
  ...
  error?: string
  // Cached data for when status is 'exists'
  parsedData?: {
    title: string
    artist: string
    content: string
...
const handleImportPaste = (event: ClipboardEvent) => {
  ...
  // Prevent default only if we're pasting multiple lines or a cifraclub URL
  if (pastedText.includes('\n') || pastedText.includes('cifraclub.com.br')) {
    event.preventDefault()
  } else {
    return // Let the input handle single non-URL pastes
  }

  const lines = pastedText.split('\n').filter(line => line.trim())

  for (const line of lines) {
    // Split by tabs or multiple spaces (for Notion/spreadsheet copy)
    ...
    // If we found a cifraclub URL, add a row
    if (cifraUrl) {
      importRows.value.push({
        cifraUrl,
        youtubeUrl,
...
const startBatchImport = async () => {
  ...
  for (let i = 0; i < importRows.value.length; i++) {
    ...
    // Skip invalid rows or already processed
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // Override YouTube URL if provided in the import row
        ...
        if (existingSong) {
          // Song exists - mark as 'exists' and cache the data for later
          row.status = 'exists'
          row.parsedData = {
            title: data.title,
            artist: data.artist,
  ...
  // Check if all succeeded (no pending left, only done/exists/error)
  const allProcessed = importRows.value.every(row =>
    row.status === 'done' || row.status === 'exists' || row.status === 'error' ||
    !row.cifraUrl.trim() || !row.cifraUrl.includes('cifraclub.com.br')
  )
  ...
  if (allDone) {
    // Clear and close after a brief moment
    setTimeout(() => {
      importRows.value = []
...
// Overwrite an existing song when user clicks on the '?' icon
...
const handleClickOutside = (event: MouseEvent) => {
  ...
  if (settingsMenuContainer.value && !settingsMenuContainer.value.contains(event.target as Node)) {
    showSettingsMenu.value = false
  }
}
